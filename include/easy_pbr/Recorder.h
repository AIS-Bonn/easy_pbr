...
public:
    Recorder();
    ~Recorder();
    void record(gl::Texture2D& tex, const std::string name, const std::string format,  const std::string path); //downloads the tex into a pbo and downlaod from the previous pbo into a cv which is queued for writing
