...
void DirWatcher::add_dir(std::string path)
{
	...
	// std::cout << "_dirst is " << _dirs.size() << '\n';
	// for (size_t i = 0; i < _dirs.size(); i++) {
	// 	std::cout << "_dirst is " << _dirst[i].file_name << '\n';
	// }
}
...
void DirWatcher::add_dir(File& dir)
{
	...
	// std::cout << "added children "<< dir.children.size() << '\n';
...
void DirWatcher::poll_files_in(std::vector<std::string>& changes, File& dir)
{
	...
	// std::cout << "info.st_mtime is " << info.st_mtime << '\n';
	// std::cout << "dir.mtime is " << dir.mtime << '\n';
